/**
 * Export all leads assigned to Tim Wilson today to Excel
 */
...
async function exportTimWilsonLeads() {
  try {
    console.log('\n' + '='.repeat(70));
    console.log('📊 EXPORTING TIM WILSON\'S LEADS FOR TODAY');
    console.log('='.repeat(70) + '\n');

    // Get Tim Wilson's user ID
    console.log('🔍 Finding Tim Wilson...\n');
    ...
    if (!users || users.length === 0) {
      console.error('❌ Tim Wilson not found in users table');
      process.exit(1);
    }

    const timWilson = users[0];
    console.log('✅ Found Tim Wilson:');
    console.log(`   Name: ${timWilson.name}`);
    console.log(`   ID: ${timWilson.id}`);
    console.log(`   Email: ${timWilson.email}\n`);
    ...
    // Get all leads assigned to Tim Wilson today
    ...
    console.log(`📋 Found ${leads.length} leads assigned to Tim Wilson today\n`);
    ...
    const worksheet = workbook.addWorksheet('Tim Wilson Leads - ' + todayStr);
    ...
    // Save file
    const fileName = `Tim_Wilson_Leads_${todayStr}.xlsx`;
