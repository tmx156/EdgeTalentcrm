/**
 * Export all leads assigned to Jack today to Excel
 */
...
async function exportJackLeads() {
  try {
    console.log('\n' + '='.repeat(70));
    console.log('📊 EXPORTING JACK\'S LEADS FOR TODAY');
    console.log('='.repeat(70) + '\n');

    // Get Jack's user ID
    console.log('🔍 Finding Jack...\n');
    ...
    if (!users || users.length === 0) {
      console.error('❌ Jack not found in users table');
      process.exit(1);
    }
    ...
    // Get all leads assigned to Jack today
    const { data: leads, error: leadsError } = await supabase
      .from('leads')
      .select('*')
      .eq('booker_id', jack.id)
    ...
    console.log(`📋 Found ${leads.length} leads assigned to Jack today\n`);
    ...
    const worksheet = workbook.addWorksheet('Jack Leads - ' + todayStr);
    ...
    // Save file
    const fileName = `Jack_Leads_${todayStr}.xlsx`;
